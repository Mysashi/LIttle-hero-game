...
public partial class PlayerView : CanvasLayer
{
    ...
    public async void _on_animation_player_animation_finished(string anim)
    {
        ...
        {
            GD.Print("done");
            playerController.Set("isAttacking",  false);
            playerController.Set("canAttack", false);
            await ToSignal(GetTree().CreateTimer(1f), "timeout");
            playerController.Set("canAttack",  true);
        }

        if ((anim == "attack_blink") || (anim == "attack_blink2"))
        {
            playerController.Set("isAttackinginBlink", false);
            playerController.Set("stopMovement", false);
